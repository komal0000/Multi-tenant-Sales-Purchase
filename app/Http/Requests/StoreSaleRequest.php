...
class StoreSaleRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        $payload['items'] = collect($payload['items'] ?? [])
            ...
            ->filter(fn (array $item) => filled($item['item_id']) || filled($item['description']) || filled($item['qty']) || filled($item['rate']))
            ->values()
            ->all();
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'items.*.line_type' => ['required', 'in:item,general'],
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            foreach ($this->input('items', []) as $index => $item) {
                if (($item['line_type'] ?? null) === 'item' && ! filled($item['item_id'] ?? null)) {
                    $validator->errors()->add("items.$index.item_id", 'Select an item for item lines.');
                }

                if (($item['line_type'] ?? null) === 'general' && ! filled($item['description'] ?? null)) {
