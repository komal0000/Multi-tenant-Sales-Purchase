...
@section('content')
    <div class="space-y-6" x-data="{ tab: 'summary' }">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Purchase Report</h1>
            <p class="text-sm text-gray-500">Summary, date wise, party wise, and date-party wise purchase breakdowns with expense categories.</p>
        </div>

        <form method="GET" action="{{ route('reports.purchases') }}" class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <div class="sales-purchase-report-filter-grid">
                <div>
                    <label for="purchase-report-party-filter" class="block text-sm font-medium text-gray-700">Party</label>
                    <select id="purchase-report-party-filter" name="party_id" class="select2 mt-1 w-full rounded-lg border border-gray-300 px-3 py-1.5 text-sm">
                ...
                <div class="flex items-center gap-2 md:pb-0.5">
                    ...
                    <a href="{{ route('reports.purchases') }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Reset</a>
                </div>
            </div>
        </form>
        ...
        @if (! $hasSearched)
            <div class="rounded-xl border border-dashed border-gray-300 bg-white p-8 text-center text-sm text-gray-500">
                Use filters and click Apply to generate the Purchase Report.
            </div>
        @else
            ...
            <section class="space-y-4" x-show="tab === 'summary'" x-cloak>
                ...
                @include('reports.partials.line-breakdowns', [
                    ...
                    'expenseWise' => $summary['expense_wise'],
                    'includeExpense' => true,
                ])
            </section>
                    ...
                    <details class="rounded-xl border border-gray-200 bg-white shadow-sm">
                        ...
                        <div class="border-t border-gray-200 p-4">
                            @include('reports.partials.line-breakdowns', [
                                ...
                                'expenseWise' => $dateRow['expense_wise'],
                                'includeExpense' => true,
                            ])
                        </div>
                    </details>
                    ...
                    <details class="rounded-xl border border-gray-200 bg-white shadow-sm">
                        ...
                        <div class="border-t border-gray-200 p-4">
                            @include('reports.partials.line-breakdowns', [
                                ...
                                'expenseWise' => $partyRow['expense_wise'],
                                'includeExpense' => true,
                            ])
                        </div>
                    </details>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            window.jQuery('#purchase-report-party-filter').select2({
                width: '100%',
