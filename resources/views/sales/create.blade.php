...
@section('content')
    @php
        ...
        $initialItems = collect(old('items', []))
            ->map(function ($item) {
                ...
                $lineType = $item['line_type'] ?? (filled($item['item_id'] ?? null) ? 'item' : 'general');
            ...
            ->filter(fn (array $item) => $item['item_id'] !== '' || $item['description'] !== '' || $item['qty'] > 0 || $item['rate'] > 0)
            ->values()
            ->all();
    ...
    <div class="space-y-5" x-data="transactionEntryForm({
        ...
        lineTypeStorageKey: 'sales-entry-line-type',
    })">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Sales Entry</h1>
            <p class="text-sm text-gray-500">Add item-wise or general lines. Item lines drive stock movement automatically.</p>
        </div>

        <form action="{{ route('sales.store') }}" method="POST" class="space-y-5" @submit="validateBeforeSubmit($event)">
            @csrf
            ...
            <div class="rounded-xl border border-gray-300 bg-white p-3 shadow-sm">
                <div class="grid gap-3 md:grid-cols-12 py-2 md:items-end">
                    ...
                    <div class="md:col-span-8">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <button type="button" data-open-quick-party-entry data-party-select-id="sale-party-select" data-quick-party-hide-opening="true" class="text-xs font-semibold text-indigo-600 hover:text-indigo-700">+ Quick Add</button>
                        </div>
                ...
                <div class="grid grid-cols-12 gap-2 ">
                    ...
                    <div class="col-span-12 md:col-span-3" x-show="draftItem.line_type === 'item'" x-cloak>
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <button type="button" data-open-quick-item-entry class="text-xs font-semibold text-indigo-600 hover:text-indigo-700">+ Quick Add</button>
                        </div>
                        <select id="sale-item-select" x-ref="itemSelect" class="select2 mt-1 w-full rounded border border-gray-300 px-2 py-1.5 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200">
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                <a href="{{ route('sales.index') }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</a>
                <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">Save Sale</button>
            </div>
        </form>
    </div>
    ...
    <script>
        function transactionEntryForm(initialState) {
            return {
                ...
                lineTypeStorageKey: String(initialState.lineTypeStorageKey || 'sales-entry-line-type'),
                message: '',
                draftItem: {
                    line_type: 'general',
                    item_id: '',
                ...
                init() {
                    ...
                    this.items = (initialState.items || [])
                        .map((item) => {
                            const lineType = item.line_type === 'item' ? 'item' : 'general';
                        ...
                        .filter((item) => item.item_id || item.description || item.qty > 0 || item.rate > 0);
                ...
                bindPartySelect() {
                    ...
                    $party.off('change.salesEntry');
                    $party.val(this.partyId).trigger('change.select2');
                    $party.on('change.salesEntry', (event) => {
                ...
                restoreDraftLineType() {
                    try {
                        ...
                        if (['item', 'general'].includes(savedLineType)) {
                ...
                applyDraftLineType(shouldFocus = true) {
                    const lineType = this.draftItem.line_type === 'item' ? 'item' : 'general';
                ...
                resetDraftItem(lineType = this.draftItem.line_type) {
                    this.draftItem = {
                        line_type: lineType === 'item' ? 'item' : 'general',
                        item_id: '',
                        description: '',
                        qty: 1,
                        rate: 0,
